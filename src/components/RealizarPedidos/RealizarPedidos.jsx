...
const RealizarPedidos = ({setPricePedido}) => {
  ...
  const [priceSelected, setPriceSelected] = useState(productos[0].price)
  const [price, setPrice] = useState(productos[0].price)
  ...
  useEffect(() => {
    const newPrice = priceSelected * number
    setPrice(newPrice)
  }, [number]);
  ...
  return (
    <div className={`${styles.container}`}>
      ...
      <select className={`${styles.select}`} id="comboBoxProducts" onChange={handleChangeProduct}>
        {productos.map((product, index) => (
          <option value={product.price}>{product.nombre}</option>
        ))}
      </select>
      ...
      <div className={`${styles.quantityPrice}`}>
        ...
        <div className={`${styles.quiantity}`}>
          ...
          <div className={`${styles.counter}`}>
            ...
            {/* SIMBOLO DE RESTA */}
            <div className={`${styles.adicionador}`} onClick={() => restador(number)}>
            ...
            <div className={`${styles.number}`} id='counter'>
              <p>{number}</p>
            </div>

            {/* SIMBOLO DE SUMA */}
            <div className={`${styles.adicionador}`} onClick={() => sumador(number)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
      ...
      <div className={`${styles.btnContainer}`}>
        ...
        <button className={`${styles.btn}`} onClick={handleRealizarPedido}>
          <p>Realizar Pedido</p>
        </button>
      </div>
